...
import { dropTrailingAssistantPrefill } from "../../open-sse/utils/assistantPrefill.ts";
...
// it, but Copilot 400s. dropTrailingAssistantPrefill() strips the trailing assistant
// message(s) before dispatch; it now lives in open-sse/utils/assistantPrefill.ts because
// Vertex's Anthropic transport needs the same guard, and both executors call the shared
...
test("dropTrailingAssistantPrefill drops a single trailing assistant message", () => {
  ...
  const out = dropTrailingAssistantPrefill(messages);
...
test("dropTrailingAssistantPrefill is null/empty safe", () => {
  ...
  assert.deepEqual(dropTrailingAssistantPrefill([]), []);
  assert.equal(dropTrailingAssistantPrefill(undefined), undefined);
  assert.equal(dropTrailingAssistantPrefill(null), null);
});
